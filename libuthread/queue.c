...
int queue_enqueue(queue_t queue, void* data) {
    ...
    2. Non-empty list: put node at the end of the queue and move the tail pointer
    there
    */
    if (queue->head == NULL) {
        queue->head = new_node;
...
int queue_dequeue(queue_t queue, void** data) {
    if (queue->head == NULL || queue == NULL || data == NULL ||
        queue->length == 0) {
...
int queue_delete(queue_t queue, void* data) {
    ...
    while (1) {
        if (currNode->data == data) {
            ...
            seems like I have to typecast them but if I do, why do I even need it to
            be void pointers?*/
...
int queue_length(queue_t queue) { return queue->length; }
