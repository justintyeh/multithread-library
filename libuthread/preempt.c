...
void preempt_start(bool preempt) {
    ...
    // SIGVTALRM - this signal typically inidcates expiration of a timer that
    // measures CPU time used by current proc. aka virtual alarm time
    ...
    if (enable) {
        ...
        // it_interval is the period between successive timer interrupts. If zero,
        // the alarm will only be sent once. it_value is the period between now and
        ...
        new_timer.it_value.tv_usec =
            10000; // will count down from 10ms and then fire SIGVTALARM
